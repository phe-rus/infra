...
function matchesRemotePattern(url: URL, pattern: RemotePattern): boolean {
    ...
    const hostname = pattern.hostname.startsWith("*.")
        ...
        : pattern.hostname
    const hostMatches = url.hostname === hostname || url.hostname.endsWith(`.${hostname}`)
...
function isAllowedTarget(url: URL, options: ImageProxyOptions): boolean {
    if (options.remotePatterns?.some((pattern) => matchesRemotePattern(url, pattern))) return true
    if (options.allowedOrigins?.includes(url.origin)) return true
    return false
}

async function fetchWithRedirectCap(url: URL, maxRedirects: number): Promise<Response> {
    let current = url
    let hops = 0
    for (;;) {
        ...
        if (hops >= maxRedirects) return new Response(null, { status: 400 })
...
function withCacheTTL(res: Response, minimumCacheTTL: number): Response {
    const headers = new Headers(res.headers)
    const existingMaxAge = Number(headers.get("cache-control")?.match(/max-age=(\d+)/)?.[1] ?? 0)
    headers.set("cache-control", `public, max-age=${Math.max(minimumCacheTTL, existingMaxAge)}`)
...
function guardSvg(res: Response, options: ImageProxyOptions): Response | null {
    const contentType = res.headers.get("content-type") ?? ""
    if (!contentType.includes("image/svg+xml")) return res
    if (!options.dangerouslyAllowSVG) return null

    const headers = new Headers(res.headers)
    headers.set("content-disposition", options.contentDispositionType ?? "attachment")
    headers.set("content-security-policy", options.contentSecurityPolicy ?? DEFAULT_CSP)
    return new Response(res.body, { status: res.status, headers })
}
...
export function createImageProxy(options: ImageProxyOptions) {
    return async function handleImageProxy(
        ...
        if (request.method !== "GET" || url.pathname !== options.path) return null
        ...
        return withEdgeCache(request, ctx, async () => {
            const maxRedirects = options.maximumRedirects ?? DEFAULT_MAX_REDIRECTS
            const maxBody = options.maximumResponseBody ?? DEFAULT_MAX_RESPONSE_BODY
            const minTTL = options.minimumCacheTTL ?? DEFAULT_MIN_CACHE_TTL

            const fetched = await fetchWithRedirectCap(targetUrl, maxRedirects)
            if (fetched.status !== 200) return fetched
            if (exceedsMaxBody(fetched, maxBody)) return new Response(null, { status: 413 })

            const svgChecked = guardSvg(fetched, options)
            if (svgChecked === null) return new Response(null, { status: 400 })
