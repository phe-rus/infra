...
- **`www/`** ("Infraccount") — the end-user "my account" app, the same idea as myaccount.google.com. It runs no auth server of its own; it's a pure client of `infra` via `better-auth/react`'s `createAuthClient`. This is where the OAuth provider's hosted pages actually live (`/sign-in`, `/create-account`, `/two-factor`, `/consent`), plus the end-user's own account management: a profile page (name, bio, avatar), Security & sign-in (2FA enrollment, passkeys, active sessions and devices with per-device sign-out), and Wallets (saved mobile-money numbers, transaction history, email receipts, a spend estimate for the month). `infra`'s `oauthProvider` config points its `loginPage`/`consentPage`/`signUp.page` here rather than hosting them itself.
