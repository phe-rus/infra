...
Infra is one TanStack Start (React 19, server-rendered) application and one Cloudflare Worker. The same Worker serves both the admin dashboard UI and the better-auth HTTP API — there's no separate backend service to run or deploy.
