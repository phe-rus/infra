...
export function infraPayment(options: InfraPaymentOptions) {
    ...
    async function getCachedCountries(): Promise<
        ...
        endpoints: {
            ...
            walletBalances: createAuthEndpoint(
                ...
                async (ctx) => {
                    ...
                    const cachedRates =
                        await options.cache.get(FX_CACHE_KEY)
                    ...
                    const targetCurrency =
                        ctx.query.currency.toUpperCase()
                    let amount = 0
            ...
            resendReceipt: createAuthEndpoint(
                ...
                async (ctx) => {
                    ...
                    const payment = await ctx.context.adapter.findOne<{
                        ...
                        model: "payment",
                        where: [
                            { field: "id", value: ctx.body.paymentId },
                        ],
                    })
                    if (
                        !payment ||
                        payment.userId !== ctx.context.session.user.id
                    ) {
                    ...
                    if (payment.status !== "completed") {
                        throw new APIError("BAD_REQUEST", {
                            message:
                                "Only completed payments have a receipt",
                        })
                    }
            ...
            removeWalletNumber: createAuthEndpoint(
                ...
                async (ctx) => {
                    ...
                    const wallet = await ctx.context.adapter.findOne<{
                        ...
                        model: "walletNumber",
                        where: [
                            { field: "id", value: ctx.body.walletId },
                        ],
                    })
            ...
            setPrimaryWalletNumber: createAuthEndpoint(
                ...
                async (ctx) => {
                    ...
                    if (wallet.status !== "verified") {
                        throw new APIError("BAD_REQUEST", {
                            message:
                                "Only a verified number can be set as primary",
                        })
                    }
            ...
            depositPayment: createAuthEndpoint(
                ...
                async (ctx) => {
                    ...
                    if (session) {
                        ...
                        const access = await resolveOAuthAccess(
                            ctx,
                            ctx.headers
                        )
                        if (!access) {
                    ...
                    const { depositId, status } = await performDeposit(
                        ctx,
                        {
                            userId,
                            clientId,
            ...
            createPaymentIntent: createAuthEndpoint(
                ...
                async (ctx) => {
                    ...
                    if (
                        !oauthClient?.redirectUris.includes(
                            ctx.body.returnUrl
                        )
                    ) {
                        throw new APIError("BAD_REQUEST", {
            ...
            getPaymentIntent: createAuthEndpoint(
                ...
                async (ctx) => {
                    ...
                    if (
                        !intent ||
                        intent.userId !== ctx.context.session.user.id
                    ) {
                        throw new APIError("NOT_FOUND", {
            ...
            confirmPaymentIntent: createAuthEndpoint(
                ...
                async (ctx) => {
                    ...
                    if (intent.status !== "created") {
                        throw new APIError("BAD_REQUEST", {
                            message:
                                "This payment has already been actioned",
                        })
                    }
            ...
            paymentWebhook: createAuthEndpoint(
                ...
                async (ctx) => {
                    ...
                    if (!referenceId) {
                        throw new APIError("BAD_REQUEST", {
                            message:
                                "Missing depositId/payoutId/refundId",
                        })
                    }
                    ...
                    if (
                        ...
                        if (
                            matchingWallet &&
                            matchingWallet.status !== "verified"
                        ) {
                            await ctx.context.adapter.update({
                                model: "walletNumber",
                                where: [
